...
const depositFrag = /* glsl */ `
uniform float uDepositAmount;

void main() {
  gl_FragColor = vec4(uDepositAmount, 0.0, 0.0, 1.0);
}
`;
...
export class PhysarumSimulation implements Simulation {
  ...
  private agentVar: any;
  private trailVar: any;
  ...
  private setupGUI(ctx: SimulationContext) {
    ...
    const agents = ctx.gui.addFolder('Agents');
    agents.add(this.params, 'agentCount', 10_000, MAX_AGENTS, 10_000).name('Count').onChange((v: number) => {
      setAU('uActiveCount', v);
      this.depositGeo.setDrawRange(0, v);
    });
    agents.add(this.params, 'sensorAngle', 0.05, Math.PI, 0.01).name('Sensor Angle').onChange((v: number) => setAU('uSensorAngle', v));
    agents.add(this.params, 'sensorDist', 1, 50, 0.5).name('Sensor Distance').onChange((v: number) => setAU('uSensorDist', v));
    agents.add(this.params, 'turnSpeed', 0.01, 2.0, 0.01).name('Turn Speed').onChange((v: number) => setAU('uTurnSpeed', v));
    agents.add(this.params, 'moveSpeed', 0.1, 5, 0.1).name('Move Speed').onChange((v: number) => setAU('uMoveSpeed', v));

    const trail = ctx.gui.addFolder('Trail');
    trail.add(this.params, 'depositAmount', 0.01, 5, 0.01).name('Deposit').onChange((v: number) => {
      this.depositMat.uniforms.uDepositAmount.value = v;
    });
    trail.add(this.params, 'decayRate', 0.5, 0.99, 0.01).name('Decay').onChange((v: number) => {
      this.trailVar.material.uniforms.uDecay.value = v;
    });
    trail.add(this.params, 'diffuseWeight', 0, 1, 0.05).name('Diffuse').onChange((v: number) => {
      this.trailVar.material.uniforms.uDiffuseWeight.value = v;
    });
    trail.add(this.params, 'brushRadius', 0.005, 0.1, 0.005).name('Brush Radius');

    const look = ctx.gui.addFolder('Appearance');
    look.add(this.params, 'exposure', 0.01, 1.0, 0.01).name('Exposure').onChange((v: number) => {
    ...
    look.addColor(this.params, 'color2').name('Low').onChange((v: string) => {
      (this.displayMat.uniforms.uColor2.value as THREE.Color).set(v);
    });
    look.addColor(this.params, 'color3').name('Mid').onChange((v: string) => {
      (this.displayMat.uniforms.uColor3.value as THREE.Color).set(v);
    });
    look.addColor(this.params, 'color4').name('High').onChange((v: string) => {
      (this.displayMat.uniforms.uColor4.value as THREE.Color).set(v);
    });
    look.add(this.params, 'brightness', 0.1, 3, 0.05).name('Brightness').onChange((v: number) => {
    ...
    pp.add(this.params, 'bloomRadius', 0, 1, 0.05).name('Bloom Radius').onChange((v: number) => {
